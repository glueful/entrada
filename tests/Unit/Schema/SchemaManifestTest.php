...
namespace Glueful\Extensions\Entrada\Tests\Unit\Schema;

use Glueful\Extensions\Entrada\Schema\EntradaSchemaVerifier;
...
final class SchemaManifestTest extends TestCase
{
    ...
    public function testVerifierClassConformsToTheManifestContract(): void
    {
        ...
        self::assertSame('glueful/entrada', (new $class())->source());
    }

    public function testVerifierCoversEveryRecursivelyDiscoveredMigration(): void
    {
        $mapped = (new EntradaSchemaVerifier())->migrationBasenames();
    ...
    {
        $g = $this->manifest();
        $candidates = ['glueful/entrada' => new \Glueful\Extensions\ExtensionCandidate(
            name: 'glueful/entrada',
            provider: $g['provider'],
        ...
        self::assertSame([], $result->errors, 'entrada must resolve enabled-alone');
    }
