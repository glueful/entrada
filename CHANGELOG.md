...
### Fixed (Apple web sign-in)
- **Apple web login no longer fails with `invalid_client` when `APPLE_CLIENT_SECRET` is a `.p8` key path.** The client-secret generator treated any value containing a dot as a pre-built JWT and returned it verbatim — every `.p8` file path contains a dot, so the file-loading branch was unreachable and the literal path was sent to Apple as the client secret. Detection is now structural (three base64url segments whose header decodes to JSON with `alg`); file paths and inline PEMs are correctly loaded and ES256-signed. Fixing reachability surfaced two latent bugs in the signing path, both fixed: DER-to-JOSE conversion read r/s after the parser had already consumed the SEQUENCE body (producing empty components), and `convertToBinary()` re-added a DER sign byte that corrupted high-bit-set signatures, then silently truncated the oversized result — oversize components now throw instead. The generated client secret is pinned by a test that openssl-verifies a real ES256 JWT against a generated P-256 key.

### Security
- **`profile_data` now stores an identity allowlist, not the full provider payload.** `linkSocialAccount()` persisted the entire social profile including the raw OAuth response (PII and whatever fields providers add in future). It now keeps only `id`, `email`, `name`, `first_name`, `last_name`, `username`, `picture`, `email_verified` — nothing in `src/` reads the column back, so this is pure exposure reduction.
- **`ASN1Parser` bounds-checks every read.** Truncated input, indefinite-length form, over-long (>4-byte) length encodings, and declared lengths past the end of the buffer now throw `RuntimeException` instead of reading out of bounds; SEQUENCE traversal descends into the constructed element so the ECDSA `seq → r → s` read pattern is correct.
- **Apple's JWKS is now cached (1h TTL) and native Sign in with Apple gains opt-in nonce binding.** Every Apple login previously fetched `https://appleid.apple.com/auth/keys` live in the verification critical path — a JWKS blip took down all Apple auth. The JWKS is now cached via the framework cache store, with a single bypass-refetch on a `kid` miss (key rotation) and graceful degradation to a direct fetch when no cache is available; failed/malformed responses are never cached. The native endpoint (`POST /auth/social/apple`) also accepts an optional `nonce` field: when the client supplies the raw nonce it bound to its SDK request, the verified token's `nonce` claim must match (`sha256(raw)` per the Apple SDK convention, or the raw value), closing the captured-token replay window. Clients that send no nonce are unaffected.
...
### Documentation
- **README aligned with actual behavior.** Added the provider verified-email matrix (Google ✓ OIDC / Facebook ✗ / GitHub ✓ via `/user/emails` / Apple ✓), the `app.url` requirement for default callback URIs (no `HTTP_HOST` fallback), the GitHub-OAuth-App PKCE limitation, the three accepted `APPLE_CLIENT_SECRET` forms, the optional native Apple `nonce` binding, JWKS caching, the per-IP rate limits (and the `->rateLimit()` builder caveat), `FACEBOOK_API_VERSION`, and the `profile_data` identity allowlist. Corrected the ASN1Parser description (ES256 client-secret signing, not ID-token validation) and removed references to the deleted `link_accounts` flag and the never-consumed `SAUTH_AUTO_REGISTER`/`SAUTH_SYNC_PROFILE`/`SOCIAL_LOGIN_DEBUG` env vars.

### Removed
- The `link_accounts` config flag — documented as controlling account linking but read nowhere; the verified-email gate is the actual control.
